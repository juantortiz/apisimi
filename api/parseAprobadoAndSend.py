...
attachs = []
emailfrom = emailto = username = password = smtp = dirtosearch =  ""
...
def init_config(configFile):
	...
	emailto = config.get('SMTP','emailto').split(',')
...
def create_mails(dia):
	msg = MIMEMultipart()
	msg["From"] = emailfrom
	msg["To"] = ", ".join(emailto)
	subject = "Resumen de simis aprobadas al dia " if (len(attachs) > 0) else "No hay simis aprobadas en el dia "
...
def parse_archivos(archivos):
	for archivo in archivos:
		salida = parse_archivo(archivo)
...
def esta_realmente_aprobada(simi):
	cnx = mysql.connector.connect(user='simi',password='Simi.T.7102', database='jbpmdb')
...
if __name__ == '__main__':
	...
	parse_archivos(archivos)
	msg = create_mails(fechas[-1])
	send_mails(msg)
